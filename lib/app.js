...
async function getSearchResults(city, state, country) {
  ...
  const response = await request.get(`https://api.weatherbit.io/v2.0/current?key=${process.env.WEATHERBIT_API_KEY}&city=${city},${state},${country}`);
  
  const userCity = response.body.data[0];
  return {
    location: userCity.location,
    country_code: userCity.country_code,
    right_now: userCity.right_now,
    weather_description: userCity.weather_description,
